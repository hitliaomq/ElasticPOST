function Plot_Slice(S, n, flag, slice_plane, flag_save, Name)
warning off
...
for i = 1:length(flag)
    ...
    ElasticPlot_2D_flag(S, n, flagi, slice_plane);
    axis equal
    axis off
    view(slice_plane);
...
function ElasticPlot_2D_flag(S, n, flag_type, slice_plane)
t = 0:2*pi/n:2*pi;
...
% hold on
p = plot3(V.*X, V.*Y, V.*Z, 'r', 'LineWidth',2);
% hold off
if flag_type == 'v' || flag_type == 'G'
    hold on
    pmin = plot3(Vmin.*X, Vmin.*Y, Vmin.*Z, 'g--', 'LineWidth',2);
    pmax = plot3(Vmax.*X, Vmax.*Y, Vmax.*Z, 'b:', 'LineWidth',2);
    hold off
...
plotcircle(slice_plane, n, r);
plotlines(slice_plane, 12, Alim);
...
function plotcircle(slice_plane, n, r)
t = 0:2*pi/n:2*pi;
...
for i = 1:n_circle
    if i == n_circle
        plot3(r(i).*X, r(i).*Y, r(i).*Z, 'k', 'LineWidth', 0.5);
...
function plotlines(slice_plane, n, r)
t = 0:2*pi/n:2*pi;
t_d = t/pi*180;
...
hold on
for i = 1:n
    plot3([0, X(i)], [0, Y(i)], [0, Z(i)], 'k:', 'LineWidth', 0.5);
    if X(i) > 0
        Align = 'left';
    else
        Align = 'right';
